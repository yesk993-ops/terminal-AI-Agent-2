"""Logging module — structured file + console logging for agent events."""
import logging
import os
...
class TellLogger:
    ...
    def log_request(self, model: str, messages: List[Dict[str, str]]) -> None:
        self.logger.info("API Request - Model: %s, Messages: %d", model, len(messages))

    def log_response(self, model: str, response: str, success: bool = True) -> None:
        self.logger.info("API Response - Model: %s, Success: %s, Response length: %d", model, success, len(response))

    def log_command(self, cmd: str, success: bool = True, error: Optional[str] = None) -> None:
        if success:
            self.logger.info("Command executed: %s", cmd)
        else:
            self.logger.error("Command failed: %s, Error: %s", cmd, error)

    def log_security_block(self, reason: str, context: str = "") -> None:
        self.logger.warning("Security blocked - Reason: %s, Context: %s", reason, context)

    def log_file_operation(self, path: str, operation: str, success: bool = True, error: Optional[str] = None) -> None:
        if success:
            self.logger.info("File operation: %s %s", operation, path)
        else:
            self.logger.error("File operation failed: %s %s, Error: %s", operation, path, error)
...
def get_logger(name: Optional[str] = None) -> TellLogger:
    global _LOGGER
    if _LOGGER is None:
        _LOGGER = TellLogger(name or "tell")
    return _LOGGER

def log_api_call(model: str, message_count: int, success: bool = True) -> None:
    get_logger().info("API Request - Model: %s, Messages: %d", model, message_count)
    if not success:
        get_logger().error("API call failed for model: %s", model)
